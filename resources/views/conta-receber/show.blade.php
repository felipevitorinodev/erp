...
@section('title', 'Conta a Receber #' . $conta->id)
@section('page_title', 'Conta a Receber #' . $conta->id)

@section('breadcrumb')
    <a href="{{ route('conta-receber.index') }}">Contas a Receber</a> / #{{ $conta->id }}
@endsection
...
@section('page_actions')
    @if(!in_array($conta->situacao, ['paga', 'cancelada'], true))
        ...
        <form method="POST" action="{{ route('conta-receber.cancelar', $conta) }}" style="display:inline;">
            @csrf
...
@section('content')
        ...
        <div class="card__body">
            <div class="form-grid form-grid--col-3">
                ...
                <div class="form-group">
                    ...
                    <span>
                        @if($conta->venda)
                            <a href="{{ route('venda.show', $conta->venda) }}">#{{ $conta->venda->numero }}</a>
                        @else
                            —
                        @endif
                    </span>
    ...
    @if(!in_array($conta->situacao, ['paga', 'cancelada'], true))
        <div class="modal-overlay" id="modal-receber" hidden>
            <div class="modal-box">
                <div class="modal-box__header">
                    <span class="card__title">Registrar Recebimento</span>
                    <button type="button" class="btn btn--ghost btn--sm" id="btn-fechar-receber">✕</button>
                </div>
                <form method="POST" action="{{ route('conta-receber.receber', $conta) }}">
                    @csrf
                    ...
                    <div class="modal-box__footer">
                        <button type="button" class="btn btn--ghost" id="btn-cancelar-receber">Fechar</button>
...
<script>
(function () {
    var modal = document.getElementById('modal-receber');
    if (!modal) return;

    function abrir() { modal.hidden = false; }
    function fechar() { modal.hidden = true; }

    var btnAbrir = document.getElementById('btn-abrir-receber');
    if (btnAbrir) btnAbrir.addEventListener('click', abrir);

    ['btn-fechar-receber', 'btn-cancelar-receber'].forEach(function (id) {
